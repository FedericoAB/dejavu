...
import type { Page, Run } from '../models/index.js'

export type AppOptions = { allowedOrigins?: string[] }

export function createApp(workflow: Workflow, token: string, options: AppOptions = {}) {
  ...
  api.get('/workspace', route(async (_req, res) => res.json(await workflow.workspace.identity())))
  api.get('/tasks', route(async (req, res) => {
    const cursor = z.string().regex(/^\d{1,7}$/).optional().parse(req.query.cursor)
    res.json(await workflow.workspace.tasks(cursor))
  }))
  api.get('/state', route((req, res) => {
  ...
  api.get('/patterns', route((req, res) => res.json(page(req, workflow.patterns()))))
  api.get('/routines', route((req, res) => res.json(page(req, workflow.routines()))))
  api.get('/routines/:id', route((req, res) => res.json(workflow.routine(z.string().parse(req.params.id)))))
  api.get('/metrics', route((_req, res) => res.json(workflow.metrics())))
  api.get('/runs', route((req, res) => res.json(page(req, [...workflow.state.runs].reverse()))))
  ...
  api.get('/runs/:id/stream', route((req, res) => {
    const id = runId(req)
  ...
  api.get('/runs/:id', route((req, res) => res.json(workflow.get(runId(req)))))
  api.post('/runs/:id/prepare', route((req, res) => {
  ...
  api.post('/runs/:id/dismiss', route((req, res) => res.json(workflow.dismiss(runId(req)))))
  api.post('/runs/:id/reject', route((req, res) => res.json(workflow.reject(runId(req)))))
  api.post('/runs/:id/approve', route(async (req, res) => {
    z.object({ approved: z.literal(true) }).strict().parse(req.body)
    res.json(await workflow.approve(runId(req)))
  }))
  api.post('/runs/:id/verify', route(async (req, res) => res.json(await workflow.verify(runId(req)))))
  api.post('/observation', route((req, res) => {

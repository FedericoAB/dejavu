...
export function Heading({ title, description, action }: { title: string; description: string; action?: ReactNode }) { return <div className={styles.heading}><div><h1>{title}</h1><p>{description}</p></div>{action}</div> }
...
export function RunList({ runs }: { runs: Run[] }) {
  if (!runs.length) return <Empty title="Todavía no hay traspasos">Abrí una tarea para preparar el primero. Cada resultado quedará registrado acá.</Empty>
...
export function Stats({ metrics }: { metrics: Metrics }) {
  return <div className={styles.stats}>{[
    ['Traspasos verificados', String(metrics.manualCompleted + metrics.assistedCompleted), 'Documentos leídos desde Ambiguous'],
    ['Con ayuda de Déjà Vu', String(metrics.assistedCompleted), 'Traspasos asistidos y verificados'],
    ['Tiempo estimado ahorrado', duration(metrics.estimatedSavedMs), 'A partir de tus vueltas manuales'],
    ['Por terminar', String(metrics.activeRuns), 'Preparación, aprobación o lectura'],
  ].map(([title, value, note]) => <div className={styles.stat} key={title}><span>{title}</span><strong>{value}</strong><small>{note}</small></div>)}</div>
}
